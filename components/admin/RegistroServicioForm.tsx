...
export default function RegistroServicioForm({ onClose, onSuccess }: Props) {
  ...
  async function buscarPorPlaca() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (vh) {
          setPlaca(vh.placa); setMarca(vh.marca || ""); setModelo(vh.modelo || "")
          setAnio(vh.anio?.toString() || ""); setColor(vh.color || "")
          setTipoVehiculo(vh.tipo || "SEDAN"); setClienteNombre(vh.clienteNombre || "")
          setClienteTelefono(vh.clienteTelefono || ""); setClienteEmail(vh.clienteEmail || "")
  ...
  const totalSeleccionado = tiposSeleccionados.reduce((sum, id) => {
    return sum + (tiposServicio.find((x) => x.id === id)?.precio ?? 0)
  }, 0)

  const duracionTotal = tiposSeleccionados.reduce((sum, id) => {
    return sum + (tiposServicio.find((x) => x.id === id)?.duracionMinutos ?? 0)
  }, 0)

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!placa || !marca) { toast.error("La placa y la marca son obligatorias"); return }
    if (!tiposSeleccionados.length) { toast.error("Selecciona al menos un tipo de servicio"); return }
    setLoading(true)
    try {
      ...
      const svcRes = await fetch("/api/servicios", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ vehiculoId: vehiculo.id, tiposServicioIds: tiposSeleccionados, operarioId: operarioId || null, observaciones }),
      })
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                ...
                <div className="flex gap-2">
                  <input value={placa}
                    ...
                    placeholder="ABC123"
                    className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 uppercase font-mono tracking-wider" required />
                ...
                {vehiculoEncontrado && (
                  <p className="text-xs text-emerald-600 mt-1 flex items-center gap-1">
                    <Check className="w-3 h-3" /> Cliente recurrente — datos completados
                  </p>
                )}
              </div>
              <div>
              ...
              <div>
                ...
                <input type="number" value={anio} onChange={(e) => setAnio(e.target.value)} placeholder="2020" min="1990" max="2030"
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                ...
                <input value={clienteNombre} onChange={(e) => setClienteNombre(e.target.value)} placeholder="Juan García"
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">Teléfono</label>
                <input value={clienteTelefono} onChange={(e) => setClienteTelefono(e.target.value)} placeholder="3001234567"
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
              ...
              {tiposServicio.map((t) => {
                ...
                return (
                  <button type="button" key={t.id} onClick={() => toggleTipo(t.id)}
                    className={`p-3 rounded-lg border-2 text-left transition-all relative ${sel ? "border-blue-600 bg-blue-50" : "border-gray-200 hover:border-blue-300"}`}>
                    {sel && (
          ...
          <div>
            <h3 className="font-semibold text-gray-800 mb-3 flex items-center gap-2 text-sm">
              ...
              Asignación <span className="text-gray-400 text-xs font-normal">(opcional)</span>
            </h3>
            <div className="grid grid-cols-1 gap-3">
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">Operario principal</label>
                ...
                {tiposSeleccionados.map((id) => {
                  ...
                  return (
                    <div key={id} className="flex justify-between text-xs text-gray-600">
                      <span>{t.nombre}</span><span>${t.precio.toLocaleString("es-CO")}</span>
                    </div>
                  )
                })}
              </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={loading || !tiposSeleccionados.length}
